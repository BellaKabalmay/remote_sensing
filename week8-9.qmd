...
The analysis results show quite clear spatial temperature variations in Central Jakarta. Areas with a high heat index (level 4–5) tend to be concentrated in areas with high building density and a predominance of impermeable surfaces such as concrete and asphalt. This aligns with the urban heat island concept, where urban areas retain more heat than areas with vegetation.

Conversely, areas with a low heat index likely have more open space or vegetation, which helps reduce temperatures through evapotranspiration. Although I didn't directly calculate NDVI, the observed patterns are quite consistent with the theory presented in the lecture. The grid approach also offers the advantage of allowing for more detailed analysis than using a single large polygon, allowing for better identification of local variations.
...
Combining multiple MODIS images yields more stable results than using a single Landsat image, as in Week 8. Time series analysis shows that temperatures in Central Jakarta are relatively stable throughout the year, with small fluctuations likely influenced by seasonal factors and urban activity. This indicates the existence of an urban heat island phenomenon, where urban areas tend to have higher temperatures than surrounding areas.Moreover, using standard deviation provides additional insights not visible from the mean alone. Areas with high standard deviation values indicate that temperatures in those areas are more dynamic or unstable, which could be linked to changes in land use or human activity. This aligns with the concept in the lecture that changes can be detected through variations in pixel values over time.
...
Compared to my practical work, the approach used is much simpler, using only the mean and standard deviation of LST data. However, this approach remains relevant because it follows the same principle: using multi-temporal data to understand change. The difference is that in SAR, changes are measured through backscatter and tested statistically (e.g., t-test), whereas in this practical work, changes are interpreted through temperature variations. Thus, this practice can be considered as a simplified version of the change detection approach, which can be further developed using more complex statistical methods such as in SAR studies.
